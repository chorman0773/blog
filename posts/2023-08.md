...
The IR is represented as a number of structs, which can be constructed and modified by plugins. The IR is then passed arround to each plugin, starting with the frontend, passing through each optimizer,
 and being consumed by the backend to generate machine code, or a lower-level IR. The structs contain abi-safe data structures provided primarily by the `xlang_abi` crate (reexported as `xlang::abi`).
Many of these types are similar to those exposed by the Rust standard library, like `Vec` and `HashMap`.
